  ...
  **文档编号**：`Phase2_DualGrid_Design_20260601_v0.b.md`  
  **状态**：已修订（Spike 验证修正）  
  **作者**：Duke / Jarvas  
  **日期**：2026-06-03  
  **前置**：第一阶段 MVP 完成（单轨验证通过）
  **修正**：R6 Spike — `active` 标记由 uint8_t 属性改为 AttributeGroup
  ...
  **验收标准**:
  ...
  N = D_v / d_v = 面元密度因子
  每个表面体素内约 N2 个面元(表面采样)
  IPW0 初始面元间距 = d_v_init ≥ 10×d_v(粗精度)
  切削后新表面面元间距 = d_v(仿真精度)
  ...
  // 注:active 标记使用 AttributeGroup(非属性),便于 PointDelete 直接消费
  struct SurfelAttributes {
      Vec3f    position;    // 相对于体素中心的局部偏移(精度 d_v)
      Vec3f    normal;      // 表面法向量
      uint8_t  precision;   // 0=COARSE(IPW0), 1=FINE(切削后)
  };

  // 活跃/非活跃标记:AttributeGroup("active")
  //   激活:  groupHandle->set(offset, true)   // 面元存在
  //   去活:  groupHandle->set(offset, false)  // 面元已切除
  //   PointDelete::deleteFromGroup(tree, "active", /*invert=*/true)
  //     即删除非 active 组的面元(R6 Spike 验证)
  ...
  流程:
    ...
    5. 为窄带表面体素注入 IPW0 粗面元(d_v_init = max(10*d_v, D_v))
    6. 记录初始内存
  ...
  对每个表面体素 (i,j,k)(|SDF| < halfWidth * D_v 且 SDF 零交叉):
    ...
    2. 在体素表面区域按 d_v_init 间距生成 N_init2 个面元
    ...
    Phase 2 - 微观切削:
      for each voxelCoord in affectedVoxels:
        ...
        for each surfel in voxel:
          if surfel not in group("active"): continue
          dist = toolSDF.eval(surfel.position_world)
          if dist <= 0:
            remove surfel from group("active")  // 剥离

    Phase 3 - 边界面元注入(N2 表面采样):
  ...
  │ v0.b │ 2026-06-03 │ §3.1: `active` 标记由 uint8_t 属性改为 AttributeGroup（R6 Spike 验证，PointDelete 兼容性） │
  └──────┴────────────┴──────────┘
