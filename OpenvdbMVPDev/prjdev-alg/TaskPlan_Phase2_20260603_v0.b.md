**文档编号**：`TaskPlan_Phase2_20260603_v0.b.md`  
**状态**：已修订（Spike 验证修正）  
**日期**：2026-06-03
  **状态**：初稿
  **日期**：2026-06-01
  ...
  TEST(DualGridAccuracy, BoundarySurfels_WithinDv) {
      ...
      // 遍历所有 FINE 精度的活跃面元
      // 注：active 为 AttributeGroup，通过 groupHandle 判断
      // 验证它们到刀具表面的距离 < d_v
      double maxError = 0;
      size_t fineCount = 0;
      forEachPoint(billet.microGrid, [&](const Vec3d& pos, uint8_t prec) {
          if (prec == FINE) {
  ...
  │ v0.b │ 2026-06-03 │ T7.5 测试用例：`active` 由 uint8_t 属性改为 AttributeGroup 判断（R6 Spike 验证） │
  └──────┴────────────┴──────────┘
